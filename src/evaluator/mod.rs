...
pub fn lisp_eval(expr: LispValue, stg: &mut LexicalVarStorage) -> LispValue {
    ...
    match expr {
        ...
        LispValue::Quote(ref elem) => match **elem {
        ...
        LispValue::Symbol(ref symb) => stg.get(symb.to_string()).unwrap_or_else(|err| {
            panic!(err);
        }),
        LispValue::ConsCell(car, cdr) => match lisp_eval(*car, stg) {
            LispValue::Builtin(ref mut b) => (**b).call(&cdr, stg),
            LispValue::Function(ref mut f) => (**f).call(&cdr, stg),
            _ => LispValue::NIL,
        },
        _ => expr.clone(),
    }
